      ...
      <ul className={`grid ${arrangement === "grid" ? "grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4": "grid-flow-row"} flex-col gap-2 overflow-y-auto`}>
        {
          Array.from({ length: 20 }).map((_, index: number) => (
            <li onClick={() => router.push(`/dashboard/jobs/${index}`)} className={`${arrangement === "grid" ? "grid-flow-row" : "grid-cols-[0.5fr_5fr_1.5fr]"} border border-slate-300 hover:cursor-pointer hover:bg-primary-pale/80 duration-300 p-4 lg:p-6 xl:p-4 grid gap-x-2 gap-y-3 lg:gap-x-3 xl:gap-x-4`}>
