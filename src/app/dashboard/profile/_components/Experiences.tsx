  ...
  return (
    <section className="border border-slate-300 p-3 lg:p4 xl:p-5 relative">
      <div className="flex flex-row gap-x-2 justify-between items-center">
        <h2 className="text-lg text-slate700 font-semibold">Experiences</h2>
              ...
              <li key={index} className="grid grid-cols-[1fr_5fr]">
                ...
                <div className="flex flex-col gap-y-2">
                  <span className="w-full flex gap-x-2 justify-between">
                    <h3 className="text-slate-700 font-semibold">{experience?.title}</h3>
                  ...
                  <p className="text-slate-400 text-xs flex flex-row gap-1.5">
                    <span className="text-slate-700 font-semibold text-xs">{experience?.companyName}</span>.
                    <span className="text-slate-500 font-light text-xs">{experience?.jobType}</span>.
                    <span className="text-slate-500 font-light text-xs">{experience?.startDate}</span>.
                    <span className="text-slate-500 font-light text-xs">{ experience.isCurrentlyWorkingHere ? "Still working here" : experience.endDate }</span>
                  </p>
                  ...
                  <ul className="">
                    {
                      experience.achievements && experience.achievements.map((acheivement: string, innerIndex: number) => (
                        <li key={innerIndex} className="text-slate-500 text-xs tracking-wide leading-5">{acheivement}</li>
                      ))
                    }
                  </ul>
                </div>
      ...
      <button onClick={handleToggleExperiences} className="text-primary font-semibold text-center">
        {
          visibleExperiencesCount === 2
            ? `Show ${experiences?.length - visibleExperiencesCount} more experiences` :
            "Hide Experiences"
        }
      </button>
    </div>
